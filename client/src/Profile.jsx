...
const Profile = () => {
  ...
  useEffect(() => {
    const getMovieCollection = async () => {
      try {
        const res = await axios.get(`http://localhost:8000/api/collection/movie/${username}`, {
  ...
  useEffect(() => {
    const getTVCollection = async () => {
      try {
        const res = await axios.get(`http://localhost:8000/api/collection/tv/${username}`, {
          headers: { token: "Bearer " + accessToken },
        });
  ...
  return (
    <div className="flex items-start justify-center h-screen  w-full bg-black text-white">
      <div className="bg-[#222] p-5 rounded shadow-md mt-[80px]">
        <h1 className="text-3xl font-semibold mb-4 text-blue-500">{username}</h1>
        <p className="text-gray-400 mb-6">Email: {email}</p>

        <div>
          <div className="my-4">
            <h2 className="text-lg font-semibold mb-2">Your Movie Collections</h2>
            <div className="grid grid-cols-2 gap-4">
              {movieCollection.length!==0 ? movieCollection.map((item, index) => (
                <a href={`collection/movie/${item.title}`} key={index} onClick={() => setCategory('movie')}>
          ...
          <h2 className="text-lg font-semibold mb-2">Your TV Show Collections</h2>
          <div className="my-4">
            <div className="grid grid-cols-2 gap-4">
              { tvCollection.length!=0 ? tvCollection.map((item, index) => (
                <a href={`collection/tv/${item.title}`} key={index} onClick={() => setCategory('tv')}>
                  <div className="p-4 border border-gray-400 rounded-md transition duration-300 hover:bg-gray-800 hover:text-white">
                    <h1 className="text-lg font-semibold">{item.title}</h1>
                  </div>
                </a>
